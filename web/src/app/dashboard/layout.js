...
export default function DashboardLayout({ children }) {
  ...
  const getPermission = (accessCode) => {
    if (accessCode === "CASE_01") {
      if (
        user?.permission_codes.includes("P001") ||
        user?.permission_codes.includes("P001") ||
        user?.permission_codes.includes("P002") ||
        user?.permission_codes.includes("P003") ||
        user?.permission_codes.includes("P004") ||
        user?.permission_codes.includes("P005")
      ) {
        return true;
      } else {
        return false;
      }
    } else if (accessCode === "USER_01") {
      if (
        user?.permission_codes.includes("P006") ||
        user?.permission_codes.includes("P007") ||
    ...
    } else if (accessCode === "BANK_01") {
      if (
        user?.permission_codes.includes("P012") ||
        user?.permission_codes.includes("P013") ||
...
const navConfig = [
  ...
  },
  {
    id: 4,
    lable: "金融機関管理",
    path: "/dashboard/bank-management",
    icon: "bi:bank",
    accessCode: "BANK_01",
  },
];
